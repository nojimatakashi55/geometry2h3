...
def fill_h3_from_wkt(wkt_str, h3_resolution, polygon_h3_contain="overlap"):
    try:
        ...
        return fill_h3_from_shapely(geom, h3_resolution=h3_resolution, polygon_h3_contain=polygon_h3_contain)

    except Exception as e:
        raise ValueError(f"Failed to fill h3 from WKT: {e}")

def fill_h3_from_wkb(wkb_str, h3_resolution, polygon_h3_contain="overlap"):
    try:
...
def fill_h3_from_geojson_dict(geojson_dict, h3_resolution, polygon_h3_contain="overlap"):
    try:
...
def fill_h3_from_geojson_str(geojson_str, h3_resolution, polygon_h3_contain="overlap"):
    try:
