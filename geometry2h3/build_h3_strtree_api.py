...
def _build_h3_strtree_from(func, *args, h3_resolution, polygon_h3_contain):
...
def build_h3_strtree_from_shapely(shape, h3_resolution, polygon_h3_contain="overlap"):
    try:
        func = lambda g, shape : g.set_shapely(shape)

        return _build_h3_strtree_from(func, shape, h3_resolution=h3_resolution, polygon_h3_contain=polygon_h3_contain)

    except Exception as e:
        raise ValueError(f"Failed to build h3 strtree from shapely: {e}")

def build_h3_strtree_from_wkt(wkt_str, h3_resolution, polygon_h3_contain="overlap"):
    try:
        geom = shapely_geometry_api.wkt_to_shapely(wkt_str)

        return build_h3_strtree_from_shapely(geom, h3_resolution=h3_resolution, polygon_h3_contain=polygon_h3_contain)

    except Exception as e:
        raise ValueError(f"Failed to build h3 strtree from WKT: {e}")

def build_h3_strtree_from_wkb(wkb_str, h3_resolution, polygon_h3_contain="overlap"):
    try:
...
def build_h3_strtree_from_geojson_dict(geojson_dict, h3_resolution, polygon_h3_contain="overlap"):
    try:
...
def build_h3_strtree_from_geojson_str(geojson_str, h3_resolution, polygon_h3_contain="overlap"):
    try:
...
def build_h3_strtree_from_polyline(encoded_str, h3_resolution, polygon_h3_contain="overlap"):
    try:
...
def build_h3_strtree_from_tile(z, x, y, h3_resolution, polygon_h3_contain="overlap"):
    try:
...
def build_h3_strtree_from_bbox(min_lat, min_lon, max_lat, max_lon, h3_resolution, polygon_h3_contain="overlap"):
    try:
...
def build_h3_strtree_from_center_radius(lat, lon, radius_meter, h3_resolution, polygon_h3_contain="overlap"):
    try:
