...
@pytest.mark.parametrize("h3_contain", [
    "overlap",  # h3 is partially contained in shape (default)
    "",
    "???",
    None
])
def test_fill_h3_from_shapely_point(h3_contain):
    shape = Point(138.3129466385537, 36.69884457953947)

    h3_list = api.fill_h3_from_shapely(shape, h3_resolution=7, h3_contain=h3_contain)
...
def test_fill_h3_from_shapely_linestring(h3_contain):
    shape = LineString([
...
def test_fill_h3_from_shapely_linearring(h3_contain):
    shape = LinearRing([
        (139.728553, 35.6197),
...
def test_fill_h3_from_shapely_polygon(h3_contain):
    shape = Polygon(
        [
            (135.51394, 34.647321),
...
def test_fill_h3_from_wkt():
    ...
    h3_list = api.fill_h3_from_wkt(wkt, h3_resolution=7, h3_contain="overlap")

    assert isinstance(h3_list, list)
    assert len(h3_list) > 0

def test_fill_h3_from_wkb():
    ...
    h3_list = api.fill_h3_from_wkb(wkb, h3_resolution=7, h3_contain="overlap")

    assert isinstance(h3_list, list)
    assert len(h3_list) > 0

def test_fill_h3_from_geojson_dict():
    geojson = {
        "type": "Polygon",
        "coordinates": [
            [
    ...
    h3_list = api.fill_h3_from_geojson_dict(geojson, h3_resolution=7, h3_contain="overlap")

    assert isinstance(h3_list, list)
    assert len(h3_list) > 0

def test_fill_h3_from_geojson_str():
    geojson_dict = {
        "type": "Polygon",
        "coordinates": [
            [
    ...
    h3_list = api.fill_h3_from_geojson_str(geojson_str, h3_resolution=7, h3_contain="overlap")
...
def test_fill_h3_from_polyline():
    ...
    h3_list = api.fill_h3_from_polyline(encoded_str, h3_resolution=7, h3_contain="overlap")
...
def test_fill_h3_from_tile():
    ...
    h3_list = api.fill_h3_from_tile(z, x, y, h3_resolution=7, h3_contain="overlap")
...
def test_fill_h3_from_bbox():
    ...
    h3_list = api.fill_h3_from_bbox(min_lat, min_lon, max_lat, max_lon, h3_resolution=7, h3_contain="overlap")
...
def test_fill_h3_from_center_radius():
    ...
    h3_list = api.fill_h3_from_center_radius(lat, lon, radius_meter, h3_resolution=7, h3_contain="overlap")
