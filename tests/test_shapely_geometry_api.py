# tests/test_shapely_geometry_api.py
import pytest
from shapely.geometry.base import BaseGeometry
import json
import polyline
import geometry2h3.shapely_geometry_api as api

def test_wkt_to_shapely():
    ...
    geom = api.wkt_to_shapely(wkt)

    assert isinstance(geom, BaseGeometry)

def test_wkb_to_shapely():
    ...
    geom = api.wkb_to_shapely(wkb)

    assert isinstance(geom, BaseGeometry)

def test_geojson_dict_to_shapely():
    geojson_dict = {
        "type": "Polygon",
        "coordinates": [
            [
    ...
    geom = api.geojson_dict_to_shapely(geojson_dict)

    assert isinstance(geom, BaseGeometry)

def test_geojson_str_to_shapely():
    geojson_dict = {
        "type": "Polygon",
        "coordinates": [
            [
    ...
    geom = api.geojson_str_to_shapely(geojson_str)

    assert isinstance(geom, BaseGeometry)

def test_bbox_to_shapely():
    min_lat = 35.6197
    min_lon = 139.6986016
    max_lat = 35.738062
    max_lon = 139.778837

    geom = api.bbox_to_shapely(min_lat, min_lon, max_lat, max_lon)
...
def test_polyline_to_shapely():
    ...
    geom = api.polyline_to_shapely(encoded_str)

    assert isinstance(geom, BaseGeometry)

def test_tile_to_shapely():
    z = 9
    x = 452
    y = 199
    geom = api.tile_to_shapely(z, x, y)

    assert isinstance(geom, BaseGeometry)

def test_center_radius_to_shapely():
    lat = 36.69894001299462
    lon = 138.31282262004888
    radius_meter = 10000
    geom = api.center_radius_to_shapely(lat, lon, radius_meter)
